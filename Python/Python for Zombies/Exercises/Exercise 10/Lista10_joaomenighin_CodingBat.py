...
# A. near_ten
# Seja um n nÃ£o negativo, retorna True se o nÃºmero estÃ¡ a distÃ¢ncia de
# pelo menos dois de um mÃºltiplo de dez. Use a funÃ§Ã£o resto da divisÃ£o.
# near_ten(12) -> True
# near_ten(17) -> False
# near_ten(19) -> True
def near_ten(n):
...
# C. luck_sum
# Soma trÃªs inteiros a, b, c
# Se aparecer um 13 ele nÃ£o conta e todos os da sua direita tambÃ©m
# lucky_sum(1, 2, 3) -> 6
# lucky_sum(1, 2, 13) -> 3
# lucky_sum(1, 13, 3) -> 1
def lucky_sum(a, b, c):
...
def end_other(a, b):
    if len(a) > len(b):
        res = a.lower()[-len(b):] == b.lower()
    elif len(a) < len(b):
        res = b.lower()[-len(a):] == a.lower()
    else:
        res = a.lower()[-len(b):] == b.lower() or b.lower()[-len(a):] == a.lower()
    return res
...
# J. sum13
# retorna a soma dos nÃºmeros de uma lista
...
def soma_na_lista(n, lista):
    for i in range(0, len(lista)):
        for j in range (i, len(lista)):
            if lista[i] != lista[j] and lista[i] + lista[j] == n:
                return True
    return False

# M.DifÃ­cil: Fila de tijolos sem usar loops
...
# Ã© possÃ­vel uma soluÃ§Ã£o sem usar for ou while
# fila_tijolos(3, 1, 8) -> True
# fila_tijolos(3, 1, 9) -> False
# fila_tijolos(3, 2, 10) -> True
def fila_tijolos(n_peq, n_gra, meta):
    if (n_gra * 5 >= meta and int (meta / 5) * 5 + n_peq >= meta) or (n_gra * 5 < meta and n_gra * 5 + n_peq >= meta):
        return True
    return False
