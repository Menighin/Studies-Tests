...
def count_prep (txt):
    count = 0
    for word in txt:
        if word[0] in ["z", "m", "b"] and not word[-1] in ["z", "m", "b"]:
            count += 1
...
def count_verb (txt):
    count = [0, 0]
    for word in txt:
        if len(word) == 7 and not word[-1] in ["z", "m", "b"]:
            count[0] += 1
            if not word[0] in ["z", "m", "b"]:
                count[1] += 1

    return count

def dEncode (txt, op):
    googleon = "zmbtshjpnwlrcxkqvdgf"
    ...
    if op == "DECODE":
        for i in range(0, len(txt)):
            for letter in txt[i]:
                try:
                    result[i] += alphabet[googleon.index(letter)]
                except:
                    result.append(alphabet[googleon.index(letter)])
    elif op == "ENCODE":
        for i in range(0, len(txt)):
            for letter in txt[i]:
                try:
                    result[i] += googleon[alphabet.index(letter)]
                except:
                    result.append(googleon[alphabet.index(letter)])
    return result
...
def numbers (txt):
    ...
    for word in txt:
        ...
        for i in range (0, len(word)):
            num += googleon.index(word[i]) * (20 ** i)
            if word.count(word[i]) > 1:
                pure = False
                break

        if num % 42 == 0 and pure:
           result.append(word)

    return result

print ("A;%d" %count_prep(txtB))
print ("B;%d" %count_verb(txtB)[0])
print ("C;%d" %count_verb(txtB)[1])
print ("D;%s" %(" ".join(dEncode(sorted(dEncode(txtB, "DECODE")), "ENCODE"))))
print ("E;%s" %" ".join(numbers(txtB)))
